...
[**delete_job**](DeployApi.md#delete_job) | **DELETE** /deploy/job/{jobPath} | delete a job
...
# **delete_job**
> SuccessData delete_job(job_path, server=server, library=library)
...
try:
    # delete a job
    api_response = api_instance.delete_job(job_path, server=server, library=library)
    pprint(api_response)
except ApiException as e:
    print("Exception when calling DeployApi->delete_job: %s\n" % e)
```
...
> object get_deployed_folders_new(format=format, folder=folder, job=job, ctm=ctm, server=server, use_array_format=use_array_format)
...
try:
    # Get deployed jobs that match the search criteria.
    api_response = api_instance.get_deployed_folders_new(format=format, folder=folder, job=job, ctm=ctm, server=server, use_array_format=use_array_format)
    pprint(api_response)
except ApiException as e:
